...
RSpec.describe "Inventories::Items", type: :request do
  ...
  describe "POST /inventories/checkouts/items/:id" do
    it "creates a checkout item and responds with turbo stream" do
      post items_inventory_checkouts_path(inventory.id),
           params: { quantity: 2 },
    ...
    it "returns 404 for inventory not belonging to client" do
      ...
      post items_inventory_checkouts_path(other_inventory.id),
           params: { quantity: 1 },
           headers: { "Accept" => "text/vnd.turbo-stream.html" }
  ...
  describe "DELETE /inventories/checkouts/items/:id" do
    let!(:item) { create(:client_checkout_item, client_checkout: draft_checkout, client_inventory: inventory, quantity: 3) }

    it "destroys the item" do
      delete item_inventory_checkouts_path(inventory.id),
             headers: { "Accept" => "text/vnd.turbo-stream.html" }
